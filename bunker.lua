...
local function button(t,x,y,w,h,label,bg,fg)
    local tw,th=t.getSize(); w=math.max(1,math.min(w,tw-x+1)); h=math.max(1,math.min(h,th-y+1))
    t.setBackgroundColor(bg); t.setTextColor(fg or colors.white)
    for yy=y,y+h-1 do if yy>=1 and yy<=th then t.setCursorPos(x,yy); t.write(string.rep(" ",w)) end end
    local ty=y+math.floor(h/2); if ty<=th then t.setCursorPos(x+math.max(0,math.floor((w-#label)/2)),ty); t.write(label:sub(1,w)) end
end
local buttons={{id="door",x=2,y=5,w=18,h=3,label="DOOR"},{id="lights",x=21,y=5,w=18,h=3,label="LIGHTS"},{id="vent",x=2,y=9,w=18,h=3,label="VENTILATION"},{id="alarm",x=21,y=9,w=18,h=3,label="ALARM"},{id="lockdown",x=2,y=13,w=37,h=3,label="EMERGENCY LOCKDOWN"},{id="clear",x=2,y=17,w=18,h=2,label="CLEAR ALARM"},{id="logs",x=21,y=17,w=18,h=2,label="EVENT LOG"}}
...
local function drawLogs(t)
    ...
    local y=4; for i=1,math.min(#log,h-6) do t.setCursorPos(2,y); t.setTextColor(i==1 and colors.yellow or colors.lightGray); t.write(log[i]:sub(1,math.max(1,w-2))); y=y+1 end
    button(t,2,h-2,18,2,"BACK",colors.gray)
end
local function drawPin(t,input)
    local w,h=t.getSize(); t.setBackgroundColor(colors.black); t.setTextColor(colors.white); t.clear(); header(t,w); center(t,5,"SECURITY ACCESS",w); center(t,7,"Enter PIN:",w); center(t,9,string.rep("*",#input),w); center(t,11,"Type PIN on keyboard",w); if h>=12 then center(t,12,"ENTER = confirm   BACKSPACE = erase",w) end
end
...
local target=monitor or term; if monitor then monitor.setTextScale(1) end
...
local function handleKey(key)
    if screen=="pin" then
        ...
        elseif key==keys.enter then
            if pin==cfg.pin then state.authenticated=true; screen="main"; addLog("Local operator authenticated")
            else state.attempts=state.attempts+1; addLog("Invalid PIN attempt"); if state.attempts>=cfg.max_attempts then lockdown("SECURITY") end end
...
local function handleTouch(x,y)
    if screen=="main" then local b=buttonAt(x,y); if b then if b.id=="logs" then screen="logs" else doAction(b.id,"LOCAL") end; if screen=="main" then drawMain(target) else drawLogs(target) end end
    elseif screen=="logs" then local _,h=target.getSize(); if y>=h-2 then screen="main"; drawMain(target) end end
end
...
while running do
    ...
    elseif name=="modem_message" and modem and ev[3]==cfg.remote_channel then local replyChannel=ev[4]; local response=remoteCommand(ev[5]); if replyChannel and replyChannel>0 then modem.transmit(replyChannel,cfg.remote_channel,response) end end
